...
function votingPower(account) {
    return new GrowInt(account.vp, {growth:account.balance/120000000000, max: 16000}).grow(new Date().getTime()).v
}

function bandwidth(account) {
    return new GrowInt(account.bw, {growth:account.balance/36000000000, max:64000}).grow(new Date().getTime()).v
}