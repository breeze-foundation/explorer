function searchEnter() {
  ...
  if (keycode == 13 || keycode == 10) searchSubmit();
}
...
function jsonToTableRecursive(json, isInner) {
  let result = '<table class="display-tbl table tbl-4 tbl-first-120 table-sm';
  if (isInner) result += " dblocks-table-inner";
  result += '">';
  for (field in json) {
    let cleanField = DOMPurify.sanitize(field);
    let val = json[field];
    if (typeof val == "object") val = jsonToTableRecursive(val, true);
    ...
    result +=
      '<tr><th scope="row">' +
      cleanField +
      '</th><td><div class="nowrap-text"><span class="text-truncate">' +
      val.replace(/\"/g, "") +
      "</span></div></td></tr>";
  }
  result += "</table>";
...
function jsonToTableNRecursive(json, isInner) {
  let result = '<div class="display-tbl"><table class="table tbl-4 table-sm';
  if (isInner) result += " dblocks-table-inner";
  result += '">';
  for (field in json) {
    let cleanField = DOMPurify.sanitize(field);
...
function testnetBadge() {
  if (window.config.isTestnet) $("#testnet-heading-badge").show();
}
